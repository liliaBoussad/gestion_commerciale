...
class GicaPlanificationClientLine(models.Model):
    ...
    def _onchange_product_tmpl_id(self):
        ...
        if self.product_tmpl_id and self.planification_id.commande_globale_id:
            ...
            valeurs = lines.mapped('conditionnement_id')
            if len(valeurs) == 1:
                self.conditionnement_id = valeurs[0]
    ...
    @api.depends('planification_id.commande_globale_id', 'product_tmpl_id', 'conditionnement_id')
    ...
    @api.depends(
        ...
        'product_tmpl_id', 'conditionnement_id',
    )
        ...
        for rec in self:
            ...
            if not bcg.line_ids.filtered(
                ...
                raise ValidationError(
                    ...
                    f'{rec.conditionnement_id.name}" n\'existe pas dans le BCG {bcg.name}.'
                )
...
class GicaPlanificationClient(models.Model):
    ...
    def action_soumettre(self):
        ...
        if not self.line_ids:
            raise ValidationError('Ajoutez au moins un produit avant de soumettre.')
        self.write({'state': 'soumise'})
        self._rattacher_planning_usine()
        self.message_post(body='Planification soumise.')
